...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (productsFound.length !== products.length)
      throw new AppError('Um ou mais produtos são inválidos');
    ...
    if (invalidProducts.length) throw new AppError('Invalid product found');

    const productsMerged = productsFound.map(product => ({
